...
def process_file(file_path, is_index=False):
    ...
    html = re.sub(r':root\{.*?\}', root_vars, html, flags=re.DOTALL)

    # Convert dark shadows back to warm light shadows
    ...
    # Convert hardcoded orange used for 'High' severity
    ...
    if is_index:
        # Override sidebar specific colors to retain the beautiful dark espresso offset sidebar
        ...
        # Clean up some components specifically for the light theme
...
process_file('cyberdo/index.html', is_index=True)
process_file('cyberdo/db_records.html', is_index=False)
print("Light Theme Core Applied.")
